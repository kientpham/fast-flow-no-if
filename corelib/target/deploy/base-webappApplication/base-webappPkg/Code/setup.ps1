...
filter info {"$(Get-Date -UFormat "%Y-%m-%d %H:%M:%S") $ENV:ENVIRONMENT base-webapp INFO $pid 0 setup.ps1 $_"}
filter error {"$(Get-Date -UFormat "%Y-%m-%d %H:%M:%S") $ENV:ENVIRONMENT base-webapp ERROR $pid 0 setup.ps1 $_"}
